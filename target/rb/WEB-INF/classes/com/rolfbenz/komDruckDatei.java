	...
	public int dateiAusgabe(String sqlString,Connection con) {
		...
		try {
			...
			while (rs.next()) {
				...
				ausgabe.write("\n");
                                if (!vecAbNr.contains((Object) String.valueOf(abNr))) {
					// Für diese AB-Nr wurden die Texte noch nicht ausgedruckt
					...
					while (rsAkt.next()) {
						ausgabe.write("AKT"+rsAkt.getString("akt_text")+"\n");	
                                        ...
                                        vecAbNr.addElement((Object) String.valueOf(abNr));
                                }
			}
		}
		catch(Exception e) {
