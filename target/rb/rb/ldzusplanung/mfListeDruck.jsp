...
<html>
<head>
    <title>Liste: Fahnen und Metragen</title>
...
/* ******** Vergabe der Sammelauftrags-Nummern ******** */
    ...
    sql += "(kom_kz_mf='M' OR kom_kz_mf='F') AND ";
...
/* ***************************************************** */
    ...
    sql += "AND (kom_kz_mf='M' OR kom_kz_mf='F') ";
    sql += "ORDER BY ";
...
// out.println(sql);
    ...
    kdd.oeffnen();
    kdd.satzEinfuegen("ZULKOM MF " + aktFabt + " 10\n");
    ...
    if (anzDatensaetze > 0) {
        ...
        sql += "kom_status=40,kom_zs_sys=30,kom_tagesmenge_dat=TO_DATE(TO_CHAR(CURRENT_DATE,'YYYY-MM-DD'),'YYYY-MM-DD') WHERE "; // Ehemals zsSys=30 - Änderung am 30.01.2008
        sql += "(kom_kz_mf='M' OR kom_kz_mf='F') AND kom_kom_kz='N' AND ";
        sql += "kom_prog NOT IN (SELECT prg_prg FROM plrv11.plr_prg_sperr) AND ";
        sql += "kom_art_nr NOT IN (SELECT art_nr FROM plrv11.plr_art_kopf WHERE art_sperr_kz ='J')";
