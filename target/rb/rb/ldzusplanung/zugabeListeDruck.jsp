...
<%
    ...
    // bdeDb  oraBde = new bdeDb("172.17.5.6","oratest","bdev10","dbbde",1521);
...
/* ******** Vergabe der Sammelauftrags-Nummern ******** */
    ...
    sql = " SELECT";
    sql += " kom_sammel_abnr,kom_sammel_rflg,kom_abnr,kom_art_nr,kom_kom_kz,";
    sql += " kom_status,kom_lager_kz,kom_zs_sys,kom_tagesmenge_dat,kom_werk_zuschn";
    sql += " FROM plrv11.zus_kommission ";
    sql += " WHERE kom_kom_kz='N' ";
    sql += " AND kom_status=10 ";
    sql += " AND kom_zs_sys=30 ";
    sql += " AND kom_art_nr = 4429900000 ";
    ...
    while (rs.next()) {
        try {
            saRflg++;
            if (gw.veraendert(rs.getString("kom_abnr")) == 1) {
                sAbNr++;
                saRflg = 1;
            }
...
/* ***************************************************** */
    ...
    kdd.oeffnen();
    kdd.satzEinfuegen("ZULKOM ZU " + aktFabt + " 10\n");
