...
//			"<OPTION VALUE=\"19,20\">MinML Fabt"+
		...
		if ((bereich.compareTo("A")==0) || (bereich.compareTo("P"))==0) {
			...
			retString += "<FORM ACTION=\"" + appRoot + "bde\"  METHOD=\"GET\"><TD>R&uuml;ckstand ML</TD><TD COLSPAN=2>\n";
			retString += "<INPUT TYPE=\"hidden\" NAME=\"modul\"    VALUE=\"planung\"     >\n";
			retString += "<INPUT TYPE=\"hidden\" NAME=\"formular\" VALUE=\"auswahl\"     >\n";
			retString += "<INPUT TYPE=\"hidden\" NAME=\"modus\"    VALUE=\"rueckstand\"  >\n";
			retString += "<INPUT TYPE=\"hidden\" NAME=\"userSicht\" VALUE=\""+bereich+"\">\n";
			retString += "<INPUT TYPE=\"hidden\" NAME=\"param\"    VALUE=\"ml\"          >\n";
			retString += iBdeDb.getHtmlSelectAllg("fs",spezSelect,"");
	...
	public String getSelektion (HttpServletRequest iRequest,int recht) {
		...
		if (iRequest.getParameter("modus").compareTo("unverplant")==0) {
			...
			sort = "23,24"; // Grundsätzlich wird nach SSN sortiert
			if (rumpf.compareTo("10144100")==0) {
				sort   = " 23,24"; // Sortierung ssp
			}
		...
		if (iRequest.getParameter("modus").compareTo("fsPlatz")==0) {
			...
			sort	= "23,24"; // Grundsätzlich wird nach SSN sortiert		
			if (rumpf.compareTo("10144100")==0) {
				sort   = " ma_ap,23,24"; // Sortierung ssp
			}
			if (rumpf.compareTo("10245100")==0) {
				sort   = " ma_ap,21,22"; // Sortierung maxMl
			}
			if (rumpf.compareTo("22245100")==0) {
				sort   = " 21,22"; // Sortierung maxML
			}	
		...
		try {
			...
			if (fertOrt.compareTo("243")==0) {
				selectList += ",d.sko_fabt,d.sko_uhrzeit,e.sko_fabt,e.sko_uhrzeit ";  // 23 - 24
				from       += ",plrv11.plr_subsys_kopf d,plrv11.plr_subsys_kopf e ";  // 25 - 26
				...
				selectList += ",d.sko_fabt,d.sko_uhrzeit ";  // 23 - 24
				...
				selectList += ",a.sko_fabt,a.sko_uhrzeit ";  // 23 - 24
			...
			tabelle += "<TD>Pf-<BR>stat.</TD><TD>Dezi</TD><TD>P-Tag</TD>"+wechselTermin+"<TD>Puffer</TD><TD>Split</TD></TR>";
			...
			while (rs.next()) {
				...
				try {
					...
					wechselFabt    = rs.getString(23);
					wechselUhrzeit = rs.getString(24);
					if (fertOrt.compareTo("243")==0) {
						senFabt    = rs.getString(25);
						senUhrzeit = rs.getString(26);
				...
				while (rs.next()) {
					...
					if (fsStatus<5) {
						...
						if (iFs==Long.parseLong("10144100000")) {
							...
							if (pfNr==10245100) poma = String.valueOf(pfPlatz).substring(8,11);
							pfr  = "  ";
							error.write("\nFS:"+iFs+"- vopo:"+vopo+"\n");
						} else {
